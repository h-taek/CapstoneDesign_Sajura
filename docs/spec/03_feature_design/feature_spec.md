...
| 구성 요소 | 설명 |
|---|---|
| 이메일 / 비밀번호 / 이름 입력 필드 | 검증 규칙: §1.2 |
| 가입 버튼 | 가입 완료 시 로그인 상태로 사업자 검증 화면 이동 |
| 로그인 링크 | 기존 계정 보유 시 `/login` 이동 |
...
| 스텝 | 구성 요소 |
|---|---|
| Step 1. 사업자 인증 (`/verify-business`) | 사업자등록번호 입력 필드, 인증 요청 버튼, 인증 결과 메시지, 사업자등록증 업로드 영역(계속사업자 확인 후 노출), 심사 대기(PENDING) 안내, 반려(REJECTED) 시 반려 사유 표시 + 재검증 진입 |
...
> 관리자 심사 정책 원본: §1.4 / PROGRESS §3 33차 (심사 큐 1화면 + 승인/반려 + 등록증 열람은 Phase 3 포함, 종합 관리도구는 Phase 11)

- `users.role = ADMIN` 전용. 점주 앱 하단 네비게이션에 노출하지 않음
- 심사 큐 기본 정렬: 신청 일시 오래된 순

| 구성 요소 | 설명 |
|---|---|
| 심사 큐 목록 | `business_status = PENDING` 매장 목록 (매장명 / 사업자등록번호 / 신청 일시) |
| 등록증 뷰어 | 업로드된 사업자등록증 열람 (ADMIN 가드 하 스트리밍) |
| 승인 버튼 | `business_status = VERIFIED` 처리 |
| 반려 버튼 + 사유 입력 | 사유 필수 입력 → `business_status = REJECTED`, 점주 화면에 사유 노출 |
