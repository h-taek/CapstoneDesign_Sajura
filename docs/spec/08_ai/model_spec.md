...
- 1차 타깃은 **매장 일 매출(1일 선행)** — **2단 구조 확정(38차, 담당자)**: §4의 메뉴별 수요는 매출 예측과 **별도의 분해 모델**(예측 매출 → 잘 팔릴 메뉴 산출)로 담당하고, 재료 소요·발주량은 레시피 BOM 전개로 결정론적 산출한다. 메뉴 레벨 직접 예측은 이력 부족으로 기각(재개장 후 신규 메뉴 49일 이하, EDA §3). 분해 모델 설계는 Phase 7.
...
> **산출 구조 (38차 확정)**: ① 매장 일 매출 예측(§3 초기 모델) → ② 메뉴별 수요 분해(**별도 모델**, Phase 7 설계) → ③ 재료 소요·발주량 = 레시피 BOM 전개(결정론적, `schema.md` 레시피 테이블).
